...
function BallModelo3D({ color, isDraggingHTML }) {
    ...
    useFrame((state, delta) => {
        ...
        if (isDraggingHTML) {
            groupRef.current.rotation.y += localDragDelta.x * 0.005
            groupRef.current.rotation.x += localDragDelta.y * 0.01
    ...
    return (
        <group ref={groupRef}>
            <primitive object={clonedScene} scale={7.6} position={[0, -2.85, 0]} />
        </group>
    )
}
...
useGLTF.preload('/models/tiny_user_aero.glb')